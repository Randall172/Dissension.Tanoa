...
private _Array1L = [];
{
	...
	if (_O isEqualType objNull) then {_pos = getPosASL _O;} else { if (_O isEqualType grpNull) then {_pos = getPosASL (leader _O);} else {if (_O isEqualType "") then {_pos = getMarkerPos _O} else {_pos = _O}; };};
	
	
	//Now we need to check EVERY OTHER OBJECT in the 2nd array.
	{
		//Find out if the object in the array is an OBJECT, STRING, or ARRAY. Then grab its position
		if (_x isEqualType objNull) then {_pos = getPosASL _x;} else { if (_x isEqualType grpNull) then {_pos = getPosASL (leader _x);} else {if (_x isEqualType "") then {_pos = getMarkerPos _x} else {_pos = _x}; };};
		
		//Find the distance between the 2 points
		private _ND = _pos distance _pos2;
