...
private _RandomTown = ObjNull;
if (_EnemyLand isEqualTo []) exitWith {};
if (_EnemyLand isEqualType ObjNull) then
{
	_RandomTown = _EnemyLand;
}
else
{
	_RandomTown = (((selectRandom _EnemyLand) select 1) select 0);
};
...
while {_SpwnCnt > 0} do
{
	private _unit = _grp createUnit [selectRandom R_BarrackLU,_AttackPos, [], 25, "FORM"];
...
private _waypoint2 = _grp addwaypoint[(getpos _Enemy),1];
