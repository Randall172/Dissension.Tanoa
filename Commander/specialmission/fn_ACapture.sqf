...
//Send the message!
["ASSAULT COMMANDER: TERRITORY TARGETTED",'#FFFFFF'] remoteExec ["MessageFramework",_CSide];
_AddNewsArray = ["Assault Target",format 
[
"
	The location at, %1 is an easy target to assault. Let's take it before they are able to use it against us!<br/>
"
...
if (_WestRun) then 
{
	...
	[_grp],
	{
			...
			if (isServer) then {[West,_Marker,_Group,"ACapture"] call DIS_fnc_mrkersave; };
...
else 
{
	...
	[_grp],
	{
			...
			if (isServer) then {[East,_Marker,_Group,"ACapture"] call DIS_fnc_mrkersave; };
