//This function will dictate how the Aggressive commander responds to threats!
...
if (_Lvl <= 2) exitWith
{
	...
	_wp2 setWaypointSpeed "NORMAL";
		...
		[_Grp,_Color,_mkr,_Cside],
		{
			...
			if (isServer) then {[_Cside,_Marker,_Group,"Aggressive"] call DIS_fnc_mrkersave; };
...
if (_Lvl <= 4) exitWith 
{
	...
	//Much like before. The Support commander will lean toward utilizing more vehicles than infantry.
	...
	{
		{
			...
			for "_i" from 1 to (round (_MaterialCost/5)) do 
			{
				if (_CrtCnt < 13) then
				{
					_CrtCnt = _CrtCnt + 1;
				private _rndU = selectRandom _InfList;	
				private _unit = _grp2 createUnit [((_rndU select 0) select 0) ,_SpwnPos, [], 0, "FORM"];
...
if (_LvL <= 11) exitWith
{
	...
	//Much like before. The Support Enthusiast will utilize more light vehicles/vehicles than infantry.
	//This threat level will see an increase in the amount of units spawned with the addition of light vehicles mixed with 1 heavy vehicle - if possible.
	//The commander will move troops to protect the targeted town, and then move out if not used later.
	//Now lets spawn in the units.
	//First we have to find a good spawn location.
