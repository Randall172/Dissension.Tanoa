...
while {_TotalLengthRounded > _NewPosition} do 
{
	...
	ResourceMapMarkerArrayStarters pushback _marker1;
};
	...
	while {_TotalLength > _NewPosition} do
		{
			...
			_NewPosition = _NewPosition + 1;
			ResourceMapMarkerArray pushback _marker1;
		};
} foreach ResourceMapMarkerArrayStarters;

{
	ResourceMapMarkerArray pushback _x;
} foreach ResourceMapMarkerArrayStarters;
	...
	
{
	...
	if (_CountAmount > 1) then {
		...
		WaterMapMarkerArray pushback _x;
	};
...
//Lets generate the random resources!
{
	...
	ResourceDisplayMarkers pushback _Marker;
	...
	_location = "Land_WoodenCrate_01_F" createVehicle _pos;
