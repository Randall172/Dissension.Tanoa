	...
	while {KOZ_ARSENALOPEN} do
	{
		...
		_CashAmount = 0;
		{
				...
				if (LIBACTIVEATED) then 
				{
					...
					if !(_UVIO) then {_Violation = true;removeUniform  player;};	
		...
		_PreviewCost = DIS_PCASH - _NewCost;
			...
			",DIS_PCASH,_TextColor,_NewCost,_PreviewCost,_TextColor2
			]);
			
			_RandomNumber cutFadeOut 0.1;
